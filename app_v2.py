...
st.markdown(
    ...
    .status-card{border-radius:12px;padding:12px 14px;border:1px solid #e2e8f0;background:#fff}
    .status-label{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;font-weight:800}
    .status-main{font-size:21px;font-weight:900;line-height:1.1;margin-top:2px}
    .status-sub{font-size:11px;color:#64748b;margin-top:4px;line-height:1.35}
    .pills{display:flex;gap:6px;flex-wrap:wrap;margin-top:7px}
    .pill{display:inline-block;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:800}
    @media (max-width:900px){.bb-title{font-size:28px}.bb-banner{flex-direction:column;align-items:flex-start}.status-main{font-size:18px}}
    </style>
    """,
...
def pagasa_rainfall_class(rain_1h: float) -> dict[str, str]:
    """PAGASA-style 1-hour heavy-rainfall classification.

    This classifies the app's rainfall value against PAGASA's published
    intensity bands. It is not a live PAGASA-issued warning/advisory.
    """
    rain = max(safe_float(rain_1h), 0.0)

    if rain >= 30.0:
        return {
            "level": "RED",
            "label": "RED",
            "title": "Torrential rainfall",
            "action": "Severe flooding is possible; monitor official advisories and local DRRMO instructions.",
            "color": "#b91c1c",
        }
    ...
    if rain >= 15.0:
        return {
            ...
            "action": "Flooding may become threatening, especially in low-lying and flood-prone areas.",
            "color": "#c2410c",
        }
    ...
    if rain >= 7.5:
        return {
            ...
            "action": "Possible flooding in low-lying areas; monitor conditions closely.",
            "color": "#a16207",
        }
    ...
    if rain >= 2.5:
        return {
            ...
            "action": "Below the PAGASA heavy-rainfall warning threshold.",
            "color": "#0369a1",
        }
...
def rainfall_intensity_name(rain_1h: float) -> str:
    rain = max(safe_float(rain_1h), 0.0)
    if rain >= 30.0:
        return "Torrential"
    if rain >= 15.0:
...
def main():
    ...
    selected_hour_rain = safe_float(
        row.get("precipitation"),
        0.0,
    )

    pagasa_current = pagasa_rainfall_class(
        rain1 if horizon > 0 else current_rain
    )

    pagasa_scenario = pagasa_rainfall_class(
        selected_hour_rain
    )
    ...
    # The flood risk must be calculated AFTER the flood model runs.
    # Rendering it earlier caused UnboundLocalError on first load.
    status_col1, status_col2 = st.columns(2)

    with status_col1:
        st.markdown(
            f"""
            <div class="status-card" style="border-left:5px solid {pagasa_current['color']};">
              <div class="status-label">PAGASA-style 1-hour rainfall classification</div>
              <div class="status-main" style="color:{pagasa_current['color']};">
                {pagasa_current['label']} · {rainfall_intensity_name(rain1 if horizon > 0 else current_rain)}
              </div>
              <div class="status-sub">
                {fmt(rain1 if horizon > 0 else current_rain, 1)} mm/h • {pagasa_current['action']}
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.caption(
            "PAGASA-style rainfall classification only: Yellow 7.5–<15 mm/h • "
            "Orange 15–<30 mm/h • Red ≥30 mm/h. "
            "This is a classification reference, not a live PAGASA-issued warning."
        )
        ...
        if point is None:
            ...
            st.markdown(
                ...
                <div class="point-card">
                  ...
                  <div class="point-row"><span class="point-label">Rainfall class</span><span class="point-value">{pagasa_scenario['label']} · {rainfall_intensity_name(selected_hour_rain)}</span></div>
    ...
    st.warning(
        "Prototype only: the flood layer is a rainfall-driven screening proxy. "
